来源：https://juejin.cn/post/6844903687593148429
...
>>
>> 1. 协议（http、https、ftp、sftp…）
>> 2. 目标url（具体的请求路径已经文件名）
>> 3. 一些必要信息（缓存、cookie之类）
>>
>>    **body包含：**
>>
>>    1. 请求的内容
...
>> 1. 外部样式会阻塞内部脚本的执行。
>>
>> 1. 外部样式与外部脚本并行加载，但外部样式会阻塞外部脚本执行。
>> 2. 如果外部脚本带有async属性，则外部脚本的加载与执行不受外部样式影响
>> 3. 如果link标签是动态创建（js生成），不管有无async属性，都不会阻塞外部脚本的加载与执行。
>>
...
![1677154259184](image/输入url到浏览器过程解释/1677154259184.png)
