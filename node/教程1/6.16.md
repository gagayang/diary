...
socket：双向数据通信，可以同时请求和发送数据

### 方案一、基于 Net 模块的 Socket 编程
...
上面的实现不管是客户端还是服务端，都依赖有node的内置模块net，但是浏览器这样的环境是没有node，所以浏览器端真正的想自己创建socket链接，只有使用html5的新特性：WebSocket，才能实现浏览器端的socket链接，服务端我们重新使用一个新包：ws
...
方案2的缺点就是websocket兼容性不太好，所以第三个方案是socket.io，可以兼容IE8
